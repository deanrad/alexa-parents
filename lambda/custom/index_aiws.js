...
const languageStrings = {
  en: {
    translation: {
      WELCOME: "Welcome to My Parents Alexa App, by Deanius Solutions!",
      HELP:
        "Say about, to hear more about the city, or say coffee, breakfast, lunch, or dinner, to hear local restaurant suggestions, or say recommend an attraction, or say, go outside. ",
      ABOUT:
        "Gloucester Massachusetts is a city on the Atlantic Ocean.  A popular summer beach destination, Gloucester has a rich history of fishing and ship building.",
...
const data = {
  ...
  restaurants: [
    ...
    },
    {
      ...
      description:
        "A homestyle diner located just across the street from the harbor sea wall."
    },
    {
      ...
      meals: "breakfast, lunch",
      description:
        "A quaint eatery, popular for weekend brunch.  Try the carrot cake pancakes."
    },
    {
      name: "Seaport Grille",
      ...
      meals: "lunch, dinner",
      description:
        "Serving seafood, steak and casual fare.  Enjoy harbor views on the deck."
    },
    {
      name: "Latitude 43",
      ...
      meals: "lunch, dinner",
      description:
        "Features artsy decor and sushi specials.  Live music evenings at the adjoining Minglewood Tavern."
    },
    {
  ...
  attractions: [
    {
      name: "Whale Watching",
      description:
        "Gloucester has tour boats that depart twice daily from Rogers street at the harbor.  Try either the 7 Seas Whale Watch, or Captain Bill and Sons Whale Watch. ",
      distance: "0"
    },
    {
      name: "Good Harbor Beach",
      description:
        "Facing the Atlantic Ocean, Good Harbor Beach has huge expanses of soft white sand that attracts hundreds of visitors every day during the summer.",
      distance: "2"
    },
    {
      name: "Rockport",
      description:
        "A quaint New England town, Rockport is famous for rocky beaches, seaside parks, lobster fishing boats, and several art studios.",
      distance: "4"
    },
    {
      name: "Fenway Park",
      description:
        "Home of the Boston Red Sox, Fenway park hosts baseball games From April until October, and is open for tours. ",
      distance: "38"
    }
  ]
};
...
let myAPI = {
  ...
  port: 443,
  path: `/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%20in%20(select%20woeid%20from%20geo.places(1)%20where%20text%3D%22${encodeURIComponent(
    data.city
  )}%2C%20${
    data.state
...
const handlers = {
  ...
  AttractionIntent: function() {
    ...
    let say =
      ...
      (attraction.distance == "0"
        ? "right downtown. "
        : attraction.distance + " miles away. Have fun! ") +
  ...
  GoOutIntent: function() {
    getWeather(weatherData => {
      let say = `The high temperature will be ${weatherData.temp} in ${
        weatherData.city
      } and conditions are forecast to be ${weatherData.conditions}.`;
  ...
  let req = https.request(myAPI, res => {
    ...
    res.on("end", () => {
      ...
      Object.keys(condMap).forEach(function(key) {
        if (
          condMap[key].indexOf(parseInt(channelObj.item.forecast.code)) !== -1
        )
          condition = key;
      });
      weatherData = {
...
function getWordsmithNarrative(data, callback) {
  let wordsmithOpts = {
    ...
    path:
      "/v1.5/projects/wordsmith-alexa-weather/templates/wordsmith-alexa-weather-template/outputs",
    headers: {
